...
class ReadMe(object):
    ...
    def __subscribeLink(self, fileName:str, url:str=None):
        ...
        if url:
            ...
            link += " [原始链接](https://raw.githubusercontent.com/moshanghuakai001/adblockfilters/main/rules/%s) |"%(fileName)
        
        for i in range(1, len(self.proxyList)):
            proxy = self.proxyList[i]
            if proxy.startswith("https://gcore.jsdelivr.net/"):
                link += " [加速链接%d](%s/moshanghuakai001/adblockfilters@main/rules/%s) |"%(i, proxy, fileName)
            else:
                link += " [加速链接%d](%s/https://raw.githubusercontent.com/moshanghuakai001/adblockfilters/main/rules/%s) |"%(i, proxy, fileName)
    ...
    def regenerate(self):
        ...
        with open(self.filename, 'a') as f:
            ...
            f.write("| 规则10 |" + self.__subscribeLink("adblocksingbox.json") + " sing-box 1.12.x json |\n")
            f.write("| 规则10' |" + self.__subscribeLink("adblocksingboxlite.json") + " sing-box 1.12.x json |\n")
            f.write("| 规则11 |" + self.__subscribeLink("adblocksingbox.srs") + " sing-box 1.12.x srs |\n")
            f.write("| 规则11' |" + self.__subscribeLink("adblocksingboxlite.srs") + " sing-box 1.12.x srs |\n")
            f.write("| 规则12 |" + self.__subscribeLink("adblockloon.list") + " Loon |\n")
            f.write("| 规则12' |" + self.__subscribeLink("adblockloonlite.list") + " Loon |\n")
            f.write("\n")
